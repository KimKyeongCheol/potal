...
/* 	document.menuCreatManageForm.authorCode.value = vAuthorCode;
   	...
   	document.menuCreatManageForm.submit(); */
	 var page = "<c:url value='/sym/mnu/mcm/EgovMenuCreatSelect.do'/>?authorCode="+vAuthorCode;
	    ...
	    window.open(url,"popupView",popupOption);
	   	...
	   	document.menuCreatManageForm.action = page;
		...
		<tbody>
			<c:forEach var="result" items="${list_menumanage}" varStatus="status">
			  <tr>
			    ...
			    <td>
			       <a class="btn02"  onclick="selectMenuCreat('<c:out value="${result.authorCode}"/>'); return false;">메뉴생성</a>
			    </td>
			  </tr>
			 </c:forEach>
		</tbody>
