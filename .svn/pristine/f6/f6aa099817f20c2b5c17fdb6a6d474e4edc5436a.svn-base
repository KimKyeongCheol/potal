...
public class EgovMberManageController {
	...
	public String selectMberList(@ModelAttribute("userSearchVO") UserDefaultVO userSearchVO, ModelMap model,  HttpSession session, HttpServletResponse response) throws Exception {
	...
	public String updateMberView_user(@ModelAttribute("searchVO") UserDefaultVO userSearchVO, Model model, HttpSession session) throws Exception {
		...
		String p_chk= (String)session.getAttribute("p_chk");
		if(p_chk != null ||p_chk =="1" ) {
...
LoginVO user =
	...
	/*20191212 마이페이지 비밃번호확인 페이지*/
	@RequestMapping(value = "/member_pw.do")
	public String member_pw_chk(@ModelAttribute("loginVO") LoginVO loginVO, HttpServletRequest request,
		...
		// 미인증 사용자에 대한 보안처리
					...
					String url="";
			    	if(!isAuthenticated) {
			    		url = "uat/uia/EgovLoginUsr";
			    	}else {
			    		
			    		url="usr/mypage/MyPw_chk";
			    	}
	...
	/*20191212 마이페이지 비밃번호확인처리후 페이지 이동처리*/
		@RequestMapping(value = "/Mber_chk.do")
		public String Mber_chk(@ModelAttribute("loginVO") LoginVO loginVO, HttpServletRequest request,
				HttpServletResponse response, ModelMap model, HttpSession session) throws Exception {
	    	...
	    	if(!isAuthenticated) {
				...
				out.println("<script>alert('로그인 정보가 올바르지 않습니다.'); location.href='"+request.getServletContext().getContextPath()+"/uat/uia/EgovLoginUsr.do';</script>");
			...
			if(resultVO != null && resultVO.getId() != null && !resultVO.getId().equals("")) {
				session.setAttribute("p_chk", p_chk);
				
				url="redirect:/MberUpdtView.do";
			}else {
				url="usr/mypage/MyPw_chk";
				...
				out.println("<script>alert('로그인 정보가 올바르지 않습니다.'); location.href='"+request.getServletContext().getContextPath()+"/member_pw.do';</script>");
	...
	public String updateCert(@ModelAttribute("mberManageVO") MberManageVO mberManageVO,
			BindingResult bindingResult, Model model, HttpServletRequest request, HttpServletResponse response, HttpSession session)
			throws Exception {
		
	/*	String sDupInfo ="";*/
		
/*		if(session.getAttribute("sDupInfo") != null ) {
			sDupInfo=(String)session.getAttribute("sDupInfo");
		}*/
		...
		out.println("<script>alert('실명인증이 완료되었습니다.'); location.href='" + request.getServletContext().getContextPath() + "/MberUpdtView.do';</script>");
	...
	/*이메일 중복 체크*/
	    ...
	    public String checkSignup(HttpServletRequest request, Model model) {
	     /*   String id = request.getParameter("id");*/
	      /*  int rowcount = userService.checkSignup(id);*/
	        ...
	        String checkEmail = request.getParameter("id");
	    /*    checkEmail = new String(checkEmail.getBytes("ISO-8859-1"), "UTF-8");*/

			if (checkEmail == null || checkEmail.equals(""))
				return "forward:/uss/umt/EgovIdDplctCnfirmView.do";

			int usedCnt = mberManageService.checkEmailDplct(checkEmail);
			
			System.out.println("usedCnt :" +usedCnt);
	        
	        return String.valueOf(usedCnt);
	    }
	   ...
	   /** 사용자페이지 비밀번호 변겅
		 ...
		 * @param userSearchVO 검색조건
		 * @param mberManageVO 일반회원수정정보(비밀번호)
		 * @return /new_Password.do
		 * @throws Exception
		 */
		@RequestMapping(value = "/new_Password.do")
		public String new_Password(ModelMap model, @RequestParam Map<String, Object> commandMap, @ModelAttribute("searchVO") UserDefaultVO userSearchVO,
