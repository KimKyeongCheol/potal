...
function fn_egov_inquire_faqlistdetail(faqId) {		
	...
	document.FaqListForm.faqId.value = faqId;	
  	document.FaqListForm.action = "<c:url value='/FaqInqireCoUpdt.do'/>";  	
  	document.FaqListForm.submit();	
						...
						<div class="base-inner">
							...
							<div class="board-list-box">
								...
								<div class="tb-01">
									...
									<table summary="자주하는 질문의대한 목록을 제공합니다.">
										...
										<tbody>
											<c:if test="${fn:length(resultList) == 0}">
												<tr>
													<td class="lt_text3" colspan="4"><spring:message
											...
											<c:forEach items="${resultList}" var="resultInfo"
												...
												<tr>
													...
													<td align="left"><a
														href="<c:url value='/uss/olh/faq/FaqInqireCoUpdt.do?faqId=${resultInfo.faqId}&amp;pageIndex=${searchVO.pageIndex}'/>"
														onclick="fn_egov_inquire_faqlistdetail('<c:url value='${resultInfo.faqId}'/>'); return false;">
															<c:out value="${resultInfo.qestnSj}" />
													</a></td>
													<td class="lt_text3"><c:out
															value="${resultInfo.inqireCo}" /></td>
													<td class="lt_text3"><fmt:formatDate
															value="${resultInfo.lastUpdtPnttm}" pattern="yyyy-MM-dd" /></td>
												</tr>
											</c:forEach>
										</tbody>
								...
								<div class="board-btm-box">
									<div class="count-box">
										<span class="tit">총 게시물 <c:out
												value="${paginationInfo.totalRecordCount}"></c:out> 건
										</span>
									</div>
									...
									<div class="search-box">
										...
										<form name="FaqListForm"
											action="<c:url value='/FaqList.do'/>"
											method="post">
											<!-- search area start-->
											...
											<input class="input" id="searchText" name="searchKeyword" type="text" size="35"
												value='<c:out value="${searchVO.searchKeyword}"/>'
												maxlength="35" title="검색어 입력"> 
											
											
												<a class="btn" href="javascript:search_Faq();">검색</a>
											<%-- <a href="<c:url value='/uss/olh/faq/FaqCnRegistView.do'/>"
												onclick="fn_egov_regist_faq(); return false;"><spring:message
													code="button.create" /></a> --%>

											<!-- search area end -->
											<input name="faqId" type="hidden" value=""> <input
