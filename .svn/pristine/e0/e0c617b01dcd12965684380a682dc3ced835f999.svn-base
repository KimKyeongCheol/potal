...
function fn_egov_confirm_qnapassword(){
	...
	if(browse == "chrome"){
		...
		window.opener.document.getElementById("writngPassword").value = ls_writngPassword; //일반적인 방법
		window.opener.document.QnaManageForm.action = "<c:url value='/QnaPasswordConfirm.do'/>";
		window.opener.document.QnaManageForm.submit();
	}else{
		getDialogArguments();
